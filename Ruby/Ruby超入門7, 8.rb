...
class Drink
   def name  ・・間に書くことで、nameメソッドはDrinkクラスのメソッドとして定義される。(P.187)
    "カフェラテ"
  end
end
drink = Drink.new
puts drink.name  #=> "カフェラテ"                    ・・Drinkクラスのオブジェクトであるnameメソッドを呼び出す(オブジェクト.メソッド)。(P.187)
puts Drink.new.name  #=> "カフェラテ"                   クラス(Drink)に定義したメソッド(name)はそのクラスに属するすべてのオブジェクト(この場合はdrink)で呼び出せる。
p drink.class  #=> Drink
